...
- Usamos 1.0s por instancia → 30 × 1.0 = 30s total (mitad del tiempo original).
...
TIEMPO_POR_INSTANCIA = 1.00   # 30 × 1.0s = 30s total (vs 60s original)
FRACCION_MULTISTART  = 0.80   # 80% → 0.80s multi-start NEH (~27 starts, mismo que original con 2s)
FRACCION_BL          = 0.15   # 15% → 0.15s BL (converge en ~0.03s, sobra margen)
                               # 10% → 0.05s ILS de limpieza
...
class Schedulling:
    ...
    def ejecutar_experimentos(self):
        ...
        for i in range(1,self.n_instancias+1):
            ...
            print(f"Inst {i}: Cmax={res.get('valor_objetivo','?')} en {round(t1-t0,3)}s")
